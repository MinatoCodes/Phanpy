...
async function getAppStateFromEmail(
  ...
  const getFbstate = require(process.env.NODE_ENV === "development"
    ? "./getFbstate1.dev.js"
    : "./getFbstate1.js");
  let code2FATemp;
  let appState;
  try {
      ...
      if (err.continue) {
        ...
        await (async function submitCode(message) {
          ...
          if (facebookAccount["2FASecret"] && tryNumber == 0) {
            switch (
            [".png", ".jpg", ".jpeg"].some((i) =>
              facebookAccount["2FASecret"].endsWith(i)
            )
            ) {
              case true:
                code2FATemp = (
                  await qr.readQrCode(
                    `${process.cwd()}/${facebookAccount["2FASecret"]}`
                  )
    ...
    const loginMobile = require("./loginMobile.js");
    ...
    const tokenData = {
      ...
      ACCESS_TOKEN_EAAD6V7: result.access_token_eaad6v7
    };

    // Ghi lại file config
    await fs.writeFile('./token.json', JSON.stringify(tokenData, null, 4));
    logger('Đã cập nhật token EAAAU và EAAD6V7!', '[ AUTO LOGIN ]');

    // Ghi appState vào account.txt
    await fs.writeFile('./account.txt', JSON.stringify(appState, null, 2));
  }
  return appState;
}
...
async function getAppStateToLogin(loginWithEmail) {
  ...
  try {
    const splitAccountText = accountText
      .replace(/\|/g, "\n")
      .split("\n")
      .map((i) => i.trim())
      .filter((i) => i);
    ...
    if (!email || !password) {
      ...
      await new Promise((resolve) => {
        ...
        function showOptions() {
          rl.output.write(
            `\r${options
              .map((option, index) =>
                index === currentOption
                  ? colors.blueBright(`${character} (${index + 1}) ${option}`)
                  : `  (${index + 1}) ${option}`
              )
              .join("\n")}\u001B`
          );
        ...
        rl.input.on("keypress", (_, key) => {
          if (key.name === "up") {
            currentOption =
              (currentOption - 1 + options.length) % options.length;
      ...
      log.info(
        "LOGIN FACEBOOK",
        getText("login", "loginWith", options[currentOption])
      );
    ...
    log.info(
      "ACCOUNT INFO",
      `Email: ${facebookAccount.email}, I_User: ${facebookAccount.i_user || "(empty)"
      }`
    );
  ...
  (function loginBot(appState) {
    ...
    login(
      ...
      async function (error, api) {
          ...
          if (facebookAccount.email && facebookAccount.password) {
            ...
            setTimeout(async function refreshCookie() {
              try {
                ...
                const appState = await getAppStateFromEmail(
                  undefined,
                  facebookAccount
                );
                ...
                writeFileSync(
                  dirAccount,
                  JSON.stringify(filterKeysAppState(appState), null, 2)
                );
                setTimeout(() => (changeFbStateByCode = false), 1000);
                log.info(
                  "REFRESH COOKIE",
        ...
        log.info(
          "BOT NICK NAME",
          global.GoatBot.config.nickNameBot || "GOAT BOT"
        );
        // ———————————————————— GBAN ————————————————————— //
        let dataGban;
        ...
        try {
          ...
          if (hasBanned == true) process.exit();
        } catch (e) {
          console.log(e);
        ...
        if (global.GoatBot.config.autoLoadScripts?.enable == true) {
              ...
              if (
                ...
                try {
                  const contentCommand =
                    global.temp.contentScripts.cmds[filename] || "";
          ...
          watch(`${process.cwd()}/scripts/events`, async (event, filename) => {
            if (filename.endsWith(".js")) {
              if (
                ignoreEvents.includes(filename) ||
              ...
              if (
                ...
                try {
                  const contentEvent =
                    global.temp.contentScripts.events[filename] || "";
                  const currentContent = readFileSync(
        ...
        // ——————————————————— DASHBOARD ——————————————————— //
        if (
          global.GoatBot.config.dashBoard?.enable == true &&
        ...
        log.master(
          "LOAD TIME",
          `${convertTime(Date.now() - global.GoatBot.startTime)}`
        );
        ...
        writeFileSync(
          ...
          JSON.stringify(global.GoatBot.configCommands, null, 2)
        );
        ...
        // const passScraping = async function (api) {
        //   var form = {
        ...
        //       console.log("Đã vượt cảnh cáo facebook thành công.", "[ success ]");
        //     }
        //   });
        // };
        ...
        async function bypassAutoBehavior(resp, jar, globalOptions, appstate, ID) {
          try {
            ...
            console.error("Lỗi khi vượt cảnh cáo Facebook.", e, "[ ERROR ]");
          }
        }
        ...
        async function callBackListen(error, event) {
          if (error) {
            ...
            if (JSON.stringify(error).includes("601051028565049")) {
              await bypassAutoBehavior(resp, jar, globalOptions, appstate);
            }
            if (
              ...
              if (global.GoatBot.config.autoRestartWhenListenMqttError)
                ...
                if (intervalCheckLiveCookieAndRelogin == false) {
                  ...
                  const interval = setInterval(async () => {
                    const cookieIsLive = await checkLiveCookie(
                      cookieString,
                      facebookAccount.userAgent
                    );
                    if (cookieIsLive) {
                      ...
                      global.GoatBot.Listening = api.listenMqtt(
                        createCallBackListen(keyListen)
                      );
                    }
                  }, 5000);
                }
          ...
          if (
            ...
            if (
              !global.GoatBot.config.whiteListMode.whiteListIds.includes(
                event.senderID
              ) &&
              !global.GoatBot.config.whiteListModeThread.whiteListThreadIds.includes(
                event.threadID
              ) &&
              // admin
              !global.GoatBot.config.adminBot.includes(event.senderID)
          ...
          if (
            ...
            if (event.participantIDs)
              event.participantIDs =
                "Array(" + event.participantIDs.length + ")";
        ...
        if (
          ...
          const PORT =
            global.GoatBot.config.dashBoard?.port ||
            (!isNaN(global.GoatBot.config.serverUptime.port) &&
              global.GoatBot.config.serverUptime.port) ||
            3001;
          ...
          try {
            ...
            nameUpTime.includes("localhost") &&
              (nameUpTime = nameUpTime.replace("https", "http"));
            await server.listen(PORT);
            log.info(
              "UPTIME",
              getText("login", "openServerUptimeSuccess", nameUpTime)
            );
        ...
        if (restartListenMqtt.enable == true) {
          ...
          const restart = setInterval(async function () {
            if (restartListenMqtt.enable == false) {
              ...
              return log.warn(
                "LISTEN_MQTT",
                getText("login", "stopRestartListenMessage")
              );
            }
            try {
              await stopListening();
